      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            {formData.format_sales === "Por bandeja" && (
              <div className="mt-2">
                <Label>Tamaño de bandeja</Label>
                <RadioGroup
                  value={formData.format_sales}
                  onValueChange={(value) => setFormData((prev) => ({ ...prev, format_sales: value }))}
                  ...
                  disabled={isSubmitting}
                >
                  {bandejaOptions.map((option) => (
                    <div key={option} className="flex items-center space-x-2">
                      <RadioGroupItem value={`Por bandeja ${option}`} id={`bandeja-${option}`} />
                      <Label htmlFor={`bandeja-${option}`} className="cursor-pointer">
                        {option}
                      </Label>
                    </div>
                  ))}
